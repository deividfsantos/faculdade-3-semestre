...
double calculateDistance(RGB img0, RGB img1);
...
int main(int argc, char* argv[])
{
    ...
    int img0_size = pic[0].height * pic[0].width;
	int img1_size = pic[1].height * pic[1].width;

	RGB colors[img1_size];
    Img imgTemp;
    int bestIndex = 0;
	for(int i = 0; i < (img1_size-1); i++){
	    double lessDist = 600;
	    removeUsedIndex(bestIndex, &img0_size, pic[0].img);
        for(int j = 0; j < img0_size-1; j++){
            double result = calculateDistance(pic[0].img[j], pic[1].img[i]);
            if(result < lessDist){
                lessDist = result;
                bestIndex = j;
            }
        }
        colors[i].r = pic[0].img[bestIndex].r;
        colors[i].g = pic[0].img[bestIndex].g;
        colors[i].b = pic[0].img[bestIndex].b;
    }

    imgTemp.height = pic[1].height;
    imgTemp.width = pic[1].width;
    imgTemp.img = colors;
...
double calculateDistance(RGB img0, RGB img1){
    int r = pow((img0.r - img1.r), 2);
    int g = pow((img0.g - img1.g), 2);
    int b = pow((img0.b - img1.b), 2);
    return sqrt(r + g + b);
}

void removeUsedIndex(int usedIndex, int *img0_size, RGB pic0[]){
    for (int c = usedIndex; c < *img0_size - 1; c++)
        pic0[c] = pic0[c+1];
    *img0_size = *img0_size - 1;
}
